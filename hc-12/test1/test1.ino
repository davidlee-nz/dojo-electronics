...
int command = 0;
boolean ready_for_command = true;
char commands[4][10] = {
                             "AT+RB",
                             "AT+RC",
                             "AT+RF",
...
void setup() {
  ...
  Serial.println("Requesting current status...");
}
...
void loop() {
  ...
  if(command < 4 && ready_for_command)
  {
    HC12.print(commands[command]);
  ...
  if (SerialEnd) {                              // Check to see if SerialEnd flag is true
    ...
    if (SerialReadBuffer.startsWith("AT")) {    // Has a command been sent from local computer
      ...
      Serial.print("Sending command...");           // Echo command to serial
...
Sending command...OK
OK
Sending command...OK+B2400
OK+B2400
Sending command...OK+RC001
OK+RC001
Sending command...OK+FU3
OK+FU3
Sending command...OK+RP:+14dBm
OK+RP:+14dBm
Sending command...www.hc01.com  HC-12_V2.4
